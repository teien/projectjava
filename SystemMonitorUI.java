...
public class SystemMonitorUI extends JFrame {
    ...
    private JLabel processesTextLabel;
    ...
    private void initializeUI() {
        ...
        processesTextLabel = createLabel("Process", 14);
...
//        cpuTemperatureLabel.setText(String.format("CPU Temp: %.1f°C", cpuTemperature));
    ...
    private void updateProcessInfo() {
        ...
        processDetails = processes.stream()
//                // kernel mode: Thời gian CPU mà tiến trình đã sử dụng trong chế độ nhân 
//                // user mode : Thời gian CPU mà tiến trình đã sử dụng trong chế độ người dùng
//                // p.getKernelTime + p.getUserTime : Tổng thời gian CPU mà tiến trình đã sử dụng
//                // p.getUpTime : Thời gian hoạt động của tiến trình
//                .map(p -> String.format(" %s, %.2f%%\n", p.getName(), 100d * (p.getKernelTime() + p.getUserTime()) / p.getUpTime()))
//                .collect(Collectors.joining());
                .map(p -> {
        double kernelTime = p.getKernelTime();
        double userTime = p.getUserTime();
        double upTime = p.getUpTime();
        double cpuUsage = 100d * (kernelTime + userTime) / upTime;
        
        System.out.printf("Tên: %s, Kernel Time: %.2f, User Time: %.2f, Up Time: %.2f, CPU: %.2f%%\n =====", 
                          p.getName(), kernelTime, userTime, upTime, cpuUsage);
        
        return String.format("Tên: %s, CPU: %.2f%%\n", p.getName(), cpuUsage);
    })
    .collect(Collectors.joining());
        processesTextLabel.setText(processDetails);
